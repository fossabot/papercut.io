...
On macOS, `sherpa-onnx-sys` downloads per-arch (`osx-x64` / `osx-arm64`) shared-library archives during the Cargo build, copies the `.dylib`s next to the dev binary, and emits an `@loader_path` rpath for `npm run tauri:dev`. The `tauri.macos.conf.json` `beforeBundleCommand` (`scripts/copy-sherpa-macos-libs.js`) stages `libsherpa-onnx-c-api.dylib` and `libonnxruntime.dylib` into a Tauri resource directory, and `src-tauri/build.rs` adds an `@loader_path/../Resources` rpath so the installed `.app` (binary in `Contents/MacOS`, resources in `Contents/Resources`) resolves the dylibs at launch. The same shared ONNX Runtime is reused by Libtashkeel via `ort::init_from`. macOS builds are produced per-architecture on `macos-13` (Intel x86_64) and `macos-14` (Apple Silicon aarch64) CI runners; the `.dmg` is currently shipped unsigned (see the README Gatekeeper note).
